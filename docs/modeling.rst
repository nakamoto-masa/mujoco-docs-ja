...
**margin**, **gap**
   2つのジオムマージン（またはギャップ）の合計が使用されます。マージンとギャップは距離プロパティであり、片側の仕様はあまり意味がないため、ジオムの優先度はここでは無視されます。
...
:at:`noise`: :at-val:`real, "0"`
   このセンサーのノイズモデルの標準偏差。バージョン3.1.4以前では、これによりセンサーにノイズが追加されていました。リリース3.1.4でこの機能は削除されました。詳細な正当化については :doc:`3.1.4 changelog <changelog>` を参照してください。それ以降のバージョンでは、この属性は後で使用するための標準偏差情報を保存するための便利な場所として機能します。

.. _sensor-cutoff:

:at:`cutoff`: :at-val:`real, "0"`
   この値が正の場合、センサー出力の絶対値を制限します。また、 :ref:`simulate.cc <saSimulate>` のセンサーデータプロットでセンサー出力を正規化するためにも使用されます。 :at:`cutoff` は :ref:`衝突センサー<collision-sensors>` では異なる意味を持つことに注意してください。

.. _sensor-nsample:

:at:`nsample`: :at-val:`int, "0"`
   :at-val:`nsample` が0より大きい場合、 :at:`nsample` スロットの時間インデックス付きリングバッファを作成します。状態の更新時に、現在のセンサーデータがタイムスタンプ ``time`` でバッファに追加され、最も古いサンプルが削除されます。履歴バッファ内の値は :ref:`mj_readSensor` を介して読み取ることができます。 :ref:`delay<sensor-delay>` と :ref:`interval<sensor-interval>` の両方の機能には、正の :at-val:`nsample` が必要です。
...
:at:`interp`: :at-val:`[zoh, linear, cubic], "zoh"`
   履歴バッファからの読み取り時に使用される補間方法。 :ref:`mj_readSensor` の ``interp`` 引数に対応します。
...
:at:`delay`: :at-val:`real, "0"`
   0より大きい場合、 ``mjData.sensordata`` のセンサー値は直接計算されるのではなく、 ``time - delay`` の時点で履歴バッファから読み取られます。正の :ref:`nsample<sensor-nsample>` が必要で、負の値にはできません。

   最も一般的なケースでは、 ``delay = nsample * timestep`` です。詳細は :ref:`ディレイ<CDelay>` を参照してください。

.. _sensor-interval:

:at:`interval`: :at-val:`real, "0 0"`
   この属性はセンサー値が再計算される頻度を制御します。シミュレーションのタイムステップよりも大きなサンプリング周期を持つセンサーのモデリングに有用です。履歴バッファ（ :ref:`nsample <sensor-nsample>` > 0）が必要です。

   この属性は2つの実数値で定義され、どちらも時間の単位で、 :at:`interval` = " :at-val:`period` :at-val:`phase` " と呼ばれます。 :at-val:`period` のみを指定することも可能で、その場合 :at-val:`phase` は0と仮定されます。

   :at-val:`period` は再計算間の間隔周期を指定します。デフォルト値の0は「すべてのシミュレーションタイムステップ」という特別な意味を持ちます。周期はタイムステップの整数倍である必要はないことに注意してください。例えば、シミュレーションのタイムステップが1.0で、 :at-val:`period` が2.5の場合、センサーは時刻0.0、3.0、5.0、8.0、10.0、13.0、...で計算され、実際の間隔は2タイムステップと3タイムステップの間で交互に変化します。 :at-val:`period` は負の値にできません。 ``period > timestep`` の値のみ意味があることに注意してください。タイムステップ以下の値はエラーにはなりませんが、単にセンサーが毎タイムステップで再計算されるだけです。

   :at-val:`phase` は :ref:`mj_resetData` での履歴バッファ初期化時にのみ効果を発揮します。 ``interval`` が使用される場合の、センサー計算とシミュレーション時間の *相対位相* を正確に制御するために、連続時間において（タイムステップの量子化を無視して）「シミュレーション開始前に」センサーが最後に計算された時刻を指定します。デフォルト値の0は " :at-val:`-period` " という特別な意味を持ち、シミュレーションの最初のタイムステップでセンサーが計算されることを指定します。先ほどの例の続きとして、タイムステップが1.0でintervalが " :at-val:`2.5 -1.5` " の場合、センサーは時刻1.0、4.0、6.0、9.0、11.0、14.0などで計算されます。 :at-val:`phase` は :math:`(-\text{period}, 0]` の範囲内でなければなりません。
...
アクチュエータとセンサーの両方が、タイムスタンプ付きサンプルを格納するリングバッファを介した時間ディレイをサポートしています。整数属性 :at:`nsample` （ :ref:`アクチュエータ<actuator-general-nsample>` 、 :ref:`センサー<sensor-nsample>` ）が正の場合、 :at:`nsample` スロットのバッファが :ref:`物理状態<siPhysicsState>` コンポーネント ``mjData.history`` に含まれ、サンプルと現在のタイムスタンプが状態の更新時にバッファに書き込まれます。

さらに実数値属性 :at:`delay` （ :ref:`アクチュエータ<actuator-general-delay>` 、 :ref:`センサー<sensor-delay>` ）が正の場合、順動力学の計算中に制御値またはセンサー値が（ ``ctrl`` から読み取られたり再計算されたりする代わりに）履歴バッファから読み取られます。正の :at:`delay` には正の :at:`nsample` が必要です。

読み取りは書き込みの前に行われるため、最小の正のディレイは :at:`delay` が実数値であるにもかかわらず、実質的に1タイムステップであることに注意してください。

エンジンでのディレイ付き読み取りは正の :at:`delay` によってトリガーされ、API関数 :ref:`mj_readCtrl` と :ref:`mj_readSensor` によって実行されます。これらの関数は ``time - delay`` の時点でバッファから読み取り、要求されたディレイを効果的に実装します。これらの関数は ``time`` を引数として取り、 :at:`nsample` が正であればいつでも使用でき、「履歴のみ」モード（ :at:`nsample` > 0、 :at:`delay` = 0）を含め、過去の値がAPIを通じてアクセスできますが、シミュレーションには影響しません。

**センサーモード**

センサーは :ref:`delay<sensor-delay>` と :ref:`interval<sensor-interval>` の両方の属性をサポートしています。
組み合わせによって動作が決まります：
...
.. list-table::
   ...
   * - = 0
     - = 0
     - 履歴のみ：毎ステップ計算され、 ``sensordata`` に書き込まれ、履歴バッファにプッシュされる
   * - > 0
     - = 0
     - ディレイ付き：毎ステップ計算され、 ``sensordata`` にはディレイ付き読み取り（バッファからの読み取り）が含まれる
   * - = 0
     - > 0
     - 周期的：intervalで計算され、 ``sensordata`` には最後に計算された値が含まれる（ディレイなし）
   * - > 0
     - > 0
     - 周期的 + ディレイ付き：intervalで計算され、 ``sensordata`` にはディレイ付き読み取り（バッファからの読み取り）が含まれる

**初期化**

履歴バッファは :ref:`mj_resetData` によって以下のように初期化されます：

- **値**: 常にゼロに初期化されます。リセット後のカスタム初期化には、 :ref:`mj_initCtrlHistory` と :ref:`mj_initSensorHistory` を呼び出してください。

- **アクチュエータのタイムスタンプ**: ``[..., -2*dt, -dt]`` 。

- :ref:`interval<sensor-interval>` なしの **センサーのタイムスタンプ**: ``[..., -2*dt, -dt]`` 。

- :ref:`interval<sensor-interval>` ありの **センサーのタイムスタンプ**: サンプルは ``dt`` ではなく ``period`` 間隔で配置されます。連続時間のタイムスタンプ ``[..., phase-2*period, phase-period, phase]`` は ``dt`` の最も近い倍数に切り上げられます。これはサンプルが計算された可能性のあるタイミングだからです。 ``phase = 0`` （デフォルト）の場合、 ``-period`` と解釈され、最初のサンプルは ``t = 0`` で計算されます。

**因果性と補間**

最も一般的な正のディレイ値は ``delay = timestep * nsample`` であり、因果性の問題のないシンプルな履歴バッファを実装します。

.. warning::

   ``delay > timestep * nsample`` の場合、バッファの最も古い境界より前のデータが読み取られ、非因果的な外挿が発生します：実際に記録される前の値を使用することになります。このシナリオはランタイムエラーにはならないため、回避するのはユーザーの責任です。

``delay < timestep * nsample`` の設定は問題なく、システム同定や確率的ディレイに有用です。これらのユースケースでは、最大可能な ``delay_max`` を選択し、 ``nsample = ceil(delay_max / timestep)`` を設定します。その後、実行時またはシステム同定時に、 :ref:`mjModel` フィールド ``actuator_delay`` または ``sensor_delay`` を ``delay_max`` を超えない限り安全に変更できます。
...
これら2つのユースケースが :at:`interp` 属性（ :ref:`アクチュエータ<actuator-general-interp>` 、 :ref:`センサー<sensor-interp>` ）を含める理由です。実世界の外因性ディレイは一般にゼロ次ホールド現象ですが、これは不連続性を意味します：ディレイの小さな変化は、タイムステップの閾値を超えるまで効果がありません。例えば、 ``dt = 0.1`` で ``nsample = 2`` の場合、 ``delay = 0.2`` と ``delay = 0.101`` には機能的な違いがなく（両方とも最も古いサンプルから読み取ります）、 ``delay = 0.101`` から ``delay = 0.1`` に変更すると閾値を超えて動作が変わります。より高次の補間を許可することで、ディレイの効果は連続的（ ``interp = linear`` ）かつ微分可能（ ``interp = cubic`` ）になります。

補間は一部のタイプのセンサー、例えば整数値を報告するセンサー（例： :ref:`insidesite<sensor-insidesite>` ）には意味がないことに注意してください。
